...
instance FromJSON FrameworkType where
  parseJSON = withText "FrameworkType" $ \v -> return (read $ T.unpack v)
...
instance FromJSON TargetPlatform where
  parseJSON = withText "TargetPlatform" $ \v -> return (read $ T.unpack v)
...
instance FromJSON Framework where
  parseJSON = withObject "Framework" $ \v -> Framework <$> v .: "name" <*> v .:? "type" .!= Dynamic <*> fmap
    nub
    (v .:? "platforms" .!= [IOS, MacOS, WatchOS, TVOS])
...
instance FromJSON Romefile where
  parseJSON = withObject "Romefile" $ \v ->
    Romefile
      <$> v
      .:  cacheJSONKey
      <*> v
      .:? repositoryMapJSONKey
      .!= []
      <*> v
      .:? ignoreMapJSONKey
      .!= []
...
instance FromJSON RomeCacheInfo where
  parseJSON =
    withObject "RomeCacheInfo" $ \v -> RomeCacheInfo <$> v .:? "s3Bucket" <*> v .:? "local" <*> v .:? "engine"
...
toRomefile ini = do
  ...
  Romefile <$> Right _cacheInfo <*> _repositoryMapEntries <*> _ignoreMapEntries <*> Right []

getSection :: T.Text -> Ini -> Either T.Text (M.HashMap T.Text T.Text)
getSection key ini = maybe (Left err) Right (M.lookup key (INI.unIni ini))
  where err = T.pack $ "Could not find section: " <> show key

getBucket :: Ini -> Either T.Text (Maybe T.Text)
getBucket ini = M.lookup s3BucketKey <$> getSection cacheSectionDelimiter ini

getLocalCacheDir :: Ini -> Either T.Text (Maybe FilePath)
getLocalCacheDir ini = fmap T.unpack . M.lookup localCacheDirKey <$> getSection cacheSectionDelimiter ini
...
getRomefileEntries sectionDelimiter ini =
  traverse toEntry . M.toList . fromMaybe M.empty . M.lookup sectionDelimiter $ INI.unIni ini
...
toFramework t = case T.splitOn "/" t of
  ...
  [fType, fName] ->
    ...
    in  left T.pack
          ...
          <*> (left (const (errorMessage unpackedName unpackedFtype)) . readEither $ unpackedFtype)
          <*> Right allPlatforms
  (fType : fNameFragments) ->
    ...
    in  left T.pack
          $   Framework
          <$> Right unpackedName
          <*> (left (const (errorMessage unpackedName unpackedFtype)) . readEither . T.unpack $ fType)
          <*> Right allPlatforms
 where
